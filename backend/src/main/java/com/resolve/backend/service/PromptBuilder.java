...
public class PromptBuilder {
    ...
    public String buildPrompt(UserDataDTO userData, String jobDescription, String latexTemplate) {
        ...
        sb.append("You are an expert resume writer and LaTeX developer. Your job is to generate a tailored, ATS-friendly, one-page resume in LaTeX format using the exact template provided.\n\n");
        ...
        sb.append("3. Tailor the bullet points to emphasize skills and experiences that match keywords and requirements from the job description.\n");
        sb.append("4. Rewrite and improve bullet points to be achievement-oriented and quantified where possible. Use the STAR method (Situation, Task, Action, Result).\n");
        sb.append("5. Prioritize the most relevant experiences, skills, and projects for this specific job. Less relevant items can be shortened or omitted if space is tight.\n");
        sb.append("6. Keep the resume to EXACTLY one page. Do not exceed one page.\n");
        sb.append("7. Output ONLY valid, compilable LaTeX code.\n");
        sb.append("8. Do NOT include any explanations, comments, markdown formatting, code fences (```), or any text before or after the LaTeX code.\n");
        sb.append("9. The first characters of your response should be \\documentclass and the last characters should be \\end{document}.\n");
        sb.append("10. Include links that are attached to entries as hyperlinks in the resume. Use the LaTeX \\href{url}{label} command to embed them naturally. For example, a certificate link can appear as '\\href{url}{Certificate}' next to the relevant entry. A project demo link can appear next to the project name. Make them look natural and professional — do not dump raw URLs. If there are too many links and space is tight, prioritize the most relevant ones for the job.\n");
        sb.append("11. Make sure all special LaTeX characters in the user's data are properly escaped (e.g. & becomes \\&, % becomes \\%, # becomes \\#).\n");
    ...
    private void appendLinksIfPresent(StringBuilder sb, List<Link> allLinks, String entityType, UUID entityId) {
        if (allLinks == null || allLinks.isEmpty() || entityId == null) return;
        ...
        for (Link link : allLinks) {
            if (entityType.equalsIgnoreCase(link.getEntityType()) && entityId.equals(link.getEntityId())) {
                ...
                String label = (link.getLabel() != null && !link.getLabel().trim().isEmpty()) ? link.getLabel().trim() : "URL";
